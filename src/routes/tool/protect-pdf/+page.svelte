<script>
    ...
    let owner = "";
    let reason = "Protected copy - do not distribute";
    ...
    async function handleProtect() {
        ...
        try {
            ...
            const watermarkText = owner.trim() || "Protected by Liqflow";
    ...
    function reset() {
        ...
        protectedUrl = null;
        owner = "";
        reason = "Protected copy - do not distribute";
        status = "upload";
    }
</script>
...
<div class="tool-page">
    {#if status === "upload"}
        <div class="container">
            <div class="header">
                <h1>Protect PDF</h1>
                <p>Add a visible protection watermark to every page.</p>
            </div>
            <FileUploader
    ...
    {:else if status === "form"}
        <div class="container form">
            <div class="header">
                <h1>Apply protection</h1>
                <p>{file?.name}</p>
            </div>
            <div class="field">
                <label for="owner">Protected for (optional)</label>
                <input
                    id="owner"
                    type="text"
                    bind:value={owner}
            ...
            <div class="field">
                <label for="reason">Footer note</label>
                <input
                    id="reason"
                    type="text"
                    bind:value={reason}
                    placeholder="Protected copy - do not distribute"
                />
            </div>
            ...
            >
                {#if isProcessing}
                    <Loader2 size={18} class="spin" /> Applying...
                {:else}
                    Apply watermark
                {/if}
            </button>
    ...
    {:else if status === "success"}
        <div class="container centered">
            ...
            <h2>Protected PDF ready</h2>
            <p>{file?.name.replace(".pdf", "-protected.pdf")}</p>
